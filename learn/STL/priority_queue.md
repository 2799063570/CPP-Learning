...
priority_queue是一种特殊的容器适配器，它提供了优先级队列的功能。优先级队列中的元素按照优先级顺序排列，默认情况下，较大的元素具有较高的优先级。
...
我们需要注意的是，插入元素时，树并不是真是存在的，只是逻辑上理解，树的结构是通过数组来存储的。每次往数组尾部插入一个元素，会对插入元素比较它在树形结构中和它的父节点的大小关系，来决定是否和父节点进行交换。
